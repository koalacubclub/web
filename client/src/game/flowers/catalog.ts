...
import type { FlowerForm, FlowerSpecies, Ink } from './types'
...
/** Sky, hill ridge and grass — just enough park to judge a patch against. */
function drawGround(
  ...
  // goes through ink(): parkInk only maps the flower palettes. Each surface picks
...
/**
 * Draw the patch belonging to tile (tx, ty) at an arbitrary spot on the canvas —
...
function place(
  ...
  opts: { species?: FlowerSpecies; form?: FlowerForm } = {},
): void {
  ctx.save()
  ctx.translate(destX - tile.x * PIXEL, destY - tile.y * PIXEL)
  drawFlowers(ctx, tile, { ...opts, ink })
  ctx.restore()
}

function renderMeadow(): void {
  const canvas = document.getElementById('meadow') as HTMLCanvasElement
  const cssW = 1080
  const cssH = 176
  const ctx = setupCanvas(canvas, cssW, cssH)
  drawGround(ctx, cssW, cssH, 74)

  // Uneven gaps and a staggered depth, per the park's own layout rules — patches
  // at a constant pitch read as a planted border, not as wildflowers.
  const xs = [0.03, 0.14, 0.235, 0.35, 0.47, 0.575, 0.69, 0.8, 0.9]
  xs.forEach((fx, i) => {
    const tile = { x: seed * 3 + i * 11, y: 4 + (i % 3) }
    place(ctx, fx * cssW, 86 + (i % 3) * 12, tile)
  })
}
...
function renderGrid(): void {
  ...
  FLOWER_SPECIES.forEach((species, si) => {
    ...
    body.innerHTML =
      `<h3>${species}</h3>` +
      `<p class="meta">form 0 · form 1 · form 0 — ${share}% of patches</p>` +
      `<p class="file">flowers/${species}.ts</p>`
    card.appendChild(body)
    host.appendChild(card)

    // Three patches of the same species: form 0, form 1, then form 0 again on
    // another tile — so both the builds and the per-patch variance show.
    const cssW = 380
    const cssH = 172
    const ctx = setupCanvas(canvas, cssW, cssH)
    drawGround(ctx, cssW, cssH, 72)
    ;([0, 1, 0] as FlowerForm[]).forEach((form, i) => {
      const tile = { x: seed * 5 + si * 13 + i * 9, y: 3 + i }
      place(ctx, 58 + i * 106, 92, tile, { species, form })
    })
  })
}
...
console.info('tile (4,6) →', flowerSpeciesAt(4, 6), 'form', flowerFormAt(4, 6))
