...
export default function ParkGame() {
  ...
  useEffect(() => {
    ...
    // scattered to build up the "koala" flower-bed lettering below.
    function drawLeaf(
      x: number,
      y: number,
      r: number,
    ...
    // "koala" spelled out on the grass beneath the IG/TikTok billboards as a dense
    // bed of leaves + camellia blossoms clustered into the letter shapes, then
    // projected onto the ground plane (vertical squash + far→near taper) so the
    // word reads as if lying flat on the floor, tilted a touch. Purely a baked
    // background: it takes no tile/collision space, so the cat walks over it.
    function drawKoalaImprint() {
      ...
      const W =
        Math.ceil(octx.measureText('koala').width) + Math.ceil(fontPx * 0.7)
      ...
      // 2. Collect jittered sample points inside the glyphs — sparse enough that
      // the grass shows through between elements (a cluster, not a solid slab).
      ...
      const petals = [COLORS.flower1, COLORS.flower3, COLORS.flower2]
      const centers = [COLORS.flower2, COLORS.white, COLORS.flower1]
      ctx.save()
      ctx.translate(cx, cy)
      ...
      for (const [lx, ly] of pts) {
        ...
        if (rng() < 0.12) {
          ...
          drawBlossom(wx, wy, r * 1.3, petals[i], centers[c])
        } else {
          const i = (rng() * leafGreens.length) | 0
          drawLeaf(wx, wy, r, (rng() - 0.5) * Math.PI, leafGreens[i], leafVein)
        }
      }
      ctx.restore()
    }
